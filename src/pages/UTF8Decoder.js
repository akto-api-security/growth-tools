...
export default function UTF8Decoder() {
    ...
    useEffect(() => {
        ...
        if(inputValue===''){
            ...
            try {
                ...
                const decodedText = new TextDecoder('utf-8').decode(new Uint8Array([...inputValue].map(c => c.charCodeAt(0))));
                setoutputValue(decodedText);
              } catch (error) {
                // Handle invalid URL encoding
                setoutputValue('Invalid UTF-8 Encoding');
                setError(true);
              }
