...
export default class ExpressionLexer {
    ...
    parseChar(str, token, charset) {
        let c = str[token.i]
        token.type = (!charset && core?.charTypes[c]) || "char"
        if (!charset && c === "/") {
    ...
    parseSquareBracket(str, token, charset) {
        ...
        if (
            ...
            if (match[1] === ":") {
                ...
                if (!core?.posixCharClasses[match[2]]) {
    ...
    parseUnicode(token, sub) {
        ...
        token.l += match ? match[0].length : 1
        token.type = "unicodecat"
        if (core?.unicodeScripts[val]) {
            token.type = "unicodescript"
        } else if (!core?.unicodeCategories[val]) {
            val = null
        }
        if (not) {
            token.type = "not" + token.type
        }
        if ((!core?.config.unicodenegated && sub[2] === "^") || !val) {
